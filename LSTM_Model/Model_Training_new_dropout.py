...
def run():
    ...
    if model_type == 2:
        model.add(LSTM(LSTM_l1_dimension, input_shape = (look_back, num_of_feautures), return_sequences=True, dropout=dropout))
        model.add(LSTM(LSTM_l2_dimension, return_sequences=False, dropout=dropout))
        

    if model_type == 3:
        model.add(LSTM(LSTM_l1_dimension, input_shape = (look_back, num_of_feautures), return_sequences=True, dropout=dropout))
        model.add(LSTM(LSTM_l2_dimension, return_sequences=True, dropout=dropout))
        model.add(LSTM(LSTM_l2_dimension, return_sequences=False, dropout=dropout))
    ...
    if model_type == 1:
        ...
        mc = ModelCheckpoint(f'LSTM_Model/Models/{to_predict_feature}-Model(dim-{LSTM_l1_dimension}_range-{y_range}_forward-{y_forward}_batch-{batchsize}_lookback-{look_back}_features-{num_of_feautures}_dropout-{dropout}).keras', 
                             monitor='val_loss', mode = 'min', verbose = 1,  save_best_only=True)

    if model_type == 2:

        mc = ModelCheckpoint(f'LSTM_Model/Models/{to_predict_feature}-Model_Type-{model_type}(dim1-{LSTM_l1_dimension}_dim2-{LSTM_l2_dimension}_range-{y_range}_forward-{y_forward}_batch-{batchsize}_lookback-{look_back}_features-{num_of_feautures}_dropout-{dropout}).keras',
                             monitor = 'val_loss', mode = 'min', verbose=1, save_best_only=True)
        
    if model_type == 3:

        mc = ModelCheckpoint(f'LSTM_Model/Models/{to_predict_feature}-Model_Type-{model_type}(dim1-{LSTM_l1_dimension}_dim2-{LSTM_l2_dimension}_dim3-{LSTM_l3_dimension}_range-{y_range}_forward-{y_forward}_batch-{batchsize}_lookback-{look_back}_features-{num_of_feautures}_dropout-{dropout}).keras',
                             monitor = 'val_loss', mode = 'min', verbose=1, save_best_only=True)
        
    if model_type == 4:
        
        mc = ModelCheckpoint(f'LSTM_Model/Models/{to_predict_feature}-Model_Type-{model_type}(dim1-{LSTM_l1_dimension}_dim2-{LSTM_l2_dimension}_dim3-{LSTM_l3_dimension}_dim4-{LSTM_l4_dimension}_range-{y_range}_forward-{y_forward}_batch-{batchsize}_lookback-{look_back}_features-{num_of_feautures}_dropout-{dropout}).keras',
                             monitor = 'val_loss', mode = 'min', verbose=1, save_best_only=True)
